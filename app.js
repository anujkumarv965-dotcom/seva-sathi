...
// Haversine (used by helpers listing)
...
/* ---------- BOOKING MANAGEMENT - robust helper listener (final) ---------- */
...
function openBookingsSection(){
  ...
  db.collection('users').doc(uid).get().then(d=>{
    ...
    if(r === 'helper'){
      show($('bookingsHelper'));
      show($('helperDashboard'));
      // attach robust listener for helper
...
// helper/customer matching utility (defensive)
function hireBelongsTo(hireField, uid){
  ...
  try{
    if(typeof hireField === 'string'){
      ...
      // last-resort: stringified match
      try{ if(JSON.stringify(hireField).includes(String(uid))) return true; }catch(e){}
    }
    return false;
  }catch(e){
...
// single unsub holder (we may attach up to 3 listeners; store them and clean)
let helperBookingsUnsubs = [];
function detachHelperBookingsListeners(){
  if(Array.isArray(helperBookingsUnsubs)){
    helperBookingsUnsubs.forEach(u => { try{ u && u(); }catch(e){} });
  }
  helperBookingsUnsubs = [];
}

function listenHelperBookings(helperUid){
  // detach previous listeners
  detachHelperBookingsListeners();

  const list = $('helperBookingsList');
  if(!list) return;
  list.innerHTML = '<div class="small-note">Loading bookings…</div>';

  // We'll run up to 3 listeners:
  // 1) where helper == uid (string)
  // 2) where helper == users/{uid} (DocumentReference)
  // 3) fallback: listen entire hires and filter client-side (only if permitted)

  // helper string query
  try {
    const q1 = db.collection('hires').where('helper','==', helperUid).orderBy('at','desc');
    const unsub1 = q1.onSnapshot(snap => {
      renderMergedHiresFromSnapshots('str', snap, helperUid);
    }, err => {
      console.warn('helper q1 snapshot error', err);
      // will not block other listeners
    });
    helperBookingsUnsubs.push(unsub1);
  } catch(e){
    console.warn('failed starting helper==uid listener', e);
  }

  // helper DocumentReference query
  try {
    const helperRef = db.collection('users').doc(helperUid);
    const q2 = db.collection('hires').where('helper','==', helperRef).orderBy('at','desc');
    const unsub2 = q2.onSnapshot(snap => {
      renderMergedHiresFromSnapshots('ref', snap, helperUid);
    }, err => {
      console.warn('helper q2 snapshot error', err);
    });
    helperBookingsUnsubs.push(unsub2);
  } catch(e){
    console.warn('failed starting helper==ref listener', e);
  }

  // fallback full-collection listener (in case storage shape is unusual)
  try {
    const q3 = db.collection('hires').orderBy('at','desc');
    const unsub3 = q3.onSnapshot(snap => {
      // render all (client-side filter will indicate matches)
      renderMergedHiresFromSnapshots('all', snap, helperUid);
    }, err => {
      console.warn('helper full hires listener error', err);
      if(err && err.code === 'permission-denied'){
        list.innerHTML = `<div class="small-note">Permission denied reading 'hires'. If you see this, your Firestore rules block reads. Check rules in Firebase console.</div>`;
      }
    });
    helperBookingsUnsubs.push(unsub3);
  } catch(e){
    console.warn('failed starting full hires listener', e);
    // try to inform user
    list.innerHTML = `<div class="small-note">Could not attach bookings listener. See console for details.</div>`;
  }
}

// We'll collect latest data from up to 3 snapshots and merge them by id.
// Simple in-memory merge storage keyed by hire id.
const hiresCache = {}; // { id: hireObj }
function renderMergedHiresFromSnapshots(sourceKey, snap, helperUid){
  // update cache entries from this snapshot
  snap.forEach(doc => {
    const d = doc.data();
    d.id = doc.id;
    // track source which updated it (for debugging)
    d._seenFrom = sourceKey;
    hiresCache[d.id] = d;
  });

  // Convert cache to array sorted by timestamp (newest first)
  const all = Object.values(hiresCache).sort((a,b)=>{
    const ta = a.at && a.at.toDate ? a.at.toDate().getTime() : (a.createdAt && a.createdAt.toDate ? a.createdAt.toDate().getTime() : 0);
    const tb = b.at && b.at.toDate ? b.at.toDate().getTime() : (b.createdAt && b.createdAt.toDate ? b.createdAt.toDate().getTime() : 0);
    return tb - ta;
  });

  // Render combined result but highlight matches
  const list = $('helperBookingsList');
  if(!list) return;
  if(all.length === 0){
    list.innerHTML = `<div class="small-note">No bookings found.</div>`;
    return;
  }
  ...
  let matchedCount = 0;
  all.forEach(b => {
    const isMatch = hireBelongsTo(b.helper, helperUid);
    if(isMatch) matchedCount++;

    const card = document.createElement('div');
    card.className = 'booking-card';
    card.style.border = isMatch ? '2px solid #2bcf6b' : '1px dashed #ddd';
    card.style.opacity = isMatch ? '1' : '0.9';

    const when = b.at && b.at.toDate ? new Date(b.at.toDate()).toLocaleString() :
                 (b.createdAt && b.createdAt.toDate ? new Date(b.createdAt.toDate()).toLocaleString() : '');

    const helperInfo = (function(){
      try{
        if(!b.helper) return '<empty>';
        if(typeof b.helper === 'string') return String(b.helper);
        if(b.helper.id) return String(b.helper.id);
        if(b.helper.path) return String(b.helper.path);
        if(b.helper.uid) return String(b.helper.uid);
        if(b.helper._id) return String(b.helper._id);
        return JSON.stringify(b.helper).slice(0,200);
      }catch(e){ return '[unreadable]'; }
    })();

    card.innerHTML = `
      <div class="booking-row">
        <div><strong>Booking:</strong> ${b.id} ${isMatch ? '• (Assigned to you)' : ''}</div>
        <div>${when}</div>
      </div>
      <div class="booking-meta">
        <div><strong>Customer:</strong> ${typeof b.customer === 'object' ? (b.customer.uid||b.customer.id||JSON.stringify(b.customer)) : (b.customer||'')}</div>
        <div><strong>Helper field:</strong> <small>${helperInfo}</small></div>
    ...
    if(isMatch){
      if(!b.status || b.status === 'requested'){
        const accept = document.createElement('button'); accept.className='btn-primary'; accept.innerText='Accept';
        accept.onclick = ()=> updateBookingStatus(b.id,'accepted',b.customer,b.helper);
        const reject = document.createElement('button'); reject.className='btn-outline'; reject.innerText='Reject';
        ...
        const complete = document.createElement('button'); complete.className='btn-primary'; complete.innerText='Complete';
        complete.onclick = ()=> updateBookingStatus(b.id,'completed',b.customer,b.helper);
        const cancel = document.createElement('button'); cancel.className='btn-outline'; cancel.innerText='Cancel';
        ...
        const info = document.createElement('div'); info.innerText='No actions available';
        actions.appendChild(info);
      }
    } else {
      const info = document.createElement('div'); info.innerText='Not assigned to you.';
      actions.appendChild(info);
    }
  ...
  summary.innerText = `Total hires found: ${all.length}. Assigned to you: ${matchedCount}.`;
  list.insertBefore(summary, list.firstChild);
}

// Customer listener (keeps previous robust behavior)
...
/* ---------- Booking rendering & actions (unchanged) ---------- */
...
// Update booking status and create notifications for both sides
...
/* ---------- Remaining features: settings, chat, reviews, notifications ---------- */

// Hide all UI sections helper
function hideAll(){
