...
/* ---------- BOOKING MANAGEMENT (debug-friendly helper listener) ---------- */
...
function openBookingsSection(){
  ...
  db.collection('users').doc(uid).get().then(d=>{
    ...
    if(r === 'helper'){
      show($('bookingsHelper'));
      show($('helperDashboard'));
      listenHelperBookings(uid);  // debug listener
    } else {
...
// DEBUG-FRIENDLY helper listener: show all hires and mark matches
...
function listenHelperBookings(helperUid){
  ...
  helperBookingsUnsub = db.collection('hires')
    ...
    .onSnapshot(async snap => {
      ...
      if(snap.empty){
        list.innerHTML = `<div class="small-note">No bookings found in 'hires' collection.</div>`;
        console.log('listenHelperBookings: hires collection is empty.');
        return;
      }
      ...
      // log every hire for debugging (helps show what "helper" field contains)
      console.log('--- All hires (debug) ---');
      all.forEach(h => {
        console.log('HIRE:', h.id, 'status:', h.status, 'createdAt:', h.createdAt, 'helper field type:', typeof h.helper, 'helper raw:', h.helper);
      });
      console.log('-------------------------');

      // Render every hire; mark those that belong to this helper
      let matchedCount = 0;
      for(const b of all){
        ...
        // highlight match with border color
        card.style.border = isMatch ? '2px solid #2bcf6b' : '1px dashed #ddd';
        card.style.opacity = isMatch ? '1' : '0.88';

        const title = document.createElement('div');
        title.className = 'booking-row';
        title.innerHTML = `<div><strong>Booking:</strong> ${b.id} ${isMatch ? '&#x1F7E2; MATCH' : ''}</div>
                           <div>${b.at && b.at.toDate ? new Date(b.at.toDate()).toLocaleString() : (b.at? new Date(b.at).toLocaleString() : '')}</div>`;

        const details = document.createElement('div');
        details.className = 'booking-meta';
        const helperInfo = (function(){
          // try to produce readable helper value
          try{
            if(!b.helper) return 'helper: <empty>';
            if(typeof b.helper === 'string') return 'helper(uid string): ' + b.helper;
            if(b.helper.id) return 'helper(DocumentRef id): ' + b.helper.id;
            if(b.helper.path) return 'helper(DocumentRef path): ' + b.helper.path;
            if(b.helper.uid) return 'helper.uid: ' + b.helper.uid;
            if(b.helper._id) return 'helper._id: ' + b.helper._id;
            return 'helper (object): ' + JSON.stringify(b.helper);
          }catch(e){
        ...
        details.innerHTML = `
          <div><strong>Customer:</strong> ${typeof b.customer === 'object' ? (b.customer.uid || (b.customer.id?b.customer.id:JSON.stringify(b.customer))) : (b.customer || '')}</div>
          <div><strong>Helper field:</strong> ${helperInfo}</div>
        ...
        // If it belongs to this helper, show normal actions; otherwise show info-only
        if(isMatch){
          ...
          const info = document.createElement('div');
          info.innerText = 'This booking does not match your user ID; shown for debugging.';
          actions.appendChild(info);
        }
      ...
      summary.innerText = `Total hires in DB: ${all.length}. Bookings matched to you: ${matchedCount}. (Green border = matched)`;
...
// Listen real-time for customer bookings — same robust approach (no debug flood)
