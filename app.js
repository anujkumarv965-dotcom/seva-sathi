...
async function updateHelperStatusUI(uid){
  ...
  const data = doc.data()||{};
  const statusText = (data.availability || 'Unknown').charAt(0).toUpperCase() + (data.availability?(''+data.availability).slice(1):'');
...
// Listen real-time for helper bookings
  ...
  helperBookingsUnsub = db.collection('hires')
    ...
    .onSnapshot(snap=>{
      ...
      snap.forEach(doc=>{
        ...
        list.appendChild(renderBookingCardForHelper(b));
      });
    });
}

// Listen real-time for customer bookings
...
function listenCustomerBookings(customerUid){
  ...
  customerBookingsUnsub = db.collection('hires')
    ...
    .onSnapshot(snap=>{
      ...
      snap.forEach(doc=>{
        const b = { id: doc.id, ...doc.data() };
        list.appendChild(renderBookingCardForCustomer(b));
      });
    });
}
...
function renderBookingCardForHelper(b){
  ...
  title.innerHTML = `<div><strong>Booking: </strong>${b.id}</div><div>${new Date(b.at?.toDate ? b.at.toDate() : Date.now()).toLocaleString()}</div>`;
...
/* ---------- existing requestHire updated (keeps previous behavior but writes hire doc with more fields) ---------- */
window.requestHire = async function(helperUid, note = ''){
  ...
  const payload = {
    ...
    status:'requested',
    price: '', // optional: you can add default price/cost here
    note: note || '',
    createdAt: firebase.firestore.FieldValue.serverTimestamp()
  };
...
/* ---------- Remaining app features (reviews, chat, notifications, etc.) ---------- */
